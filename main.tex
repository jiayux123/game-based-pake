...
\end{enumerate}
Crucially, this cycle does not require $\adv$ to send $\Reveal$ or $\Test$, i.e., $\sim$ can ``help'' $\widehat{\adv}$ gain some information about $\pw$ without $\widehat{\adv}$ performing an active attack (per the KOY notion). This is exactly what our counterexample in \Cref{sec:counterexample} exploits.

On the other hand, if we take the BPR notion (where a $\Send$ command already counts as an active attack), then the cycle above still requires an active attack (assuming $\SpuriousGuess$ does not happen). In this way we recover the security proof of UC-security $\Rightarrow$ BPR-security \cite{EC:CHKLM05}.
