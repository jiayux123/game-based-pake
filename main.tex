...
This leaves $\Send$ instances. {\color{red}In BPR-security, all $\Send$ instances are considered online attacked}; {\color{blue}in KOY-security, a $\Send$ instance is attacked only if $\adv$ also $\Reveal$s or $\Test$s it}. (The reader should pause a bit here and think about what this difference actually means; it should become clear after seeing the counterexample in \Cref{sec:counterexample}, and we reveal our understanding in \Cref{sec:conclusion}.) With the concept of ``online attacks'' defined, we can finally state our security definition for PAKE.
\begin{definition}
