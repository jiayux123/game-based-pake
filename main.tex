...
\begin{enumerate}
  ...
  \item $P$ and $P'$ output $H(K)$ and $H(K')$, respectively. Here, $H$ and $H_0$ are two ROs from $\{0,1\}^\lambda$ to itself.
\end{enumerate}
\begin{figure}[H]
\pseudocodeblock{
P(\pw) \< \< P'(\pw') \\ 
...
\begin{enumerate}
  \item Initiate $P$'s instance $P^i$ by sending $\Send(P^i, (P')^j, \text{start})$ to the game challenger.
  \item On $c$ from the game challenger, sample $y \gets \Z_p$ and
      \begin{enumerate}[(i)]
        \item Compute $Y^* := \ICEnc(\pw_i, g^y)$ and send $\Send((P')^j, P^i, Y^*)$ to the game challenger.
...
Note that {\color{blue}$\adv$ sends no $\Reveal$ command and a single $\Test$ command, so it only does one online attack and its ``baseline'' winning probability is $1/2 + 1/2|D|$}. However, we show that $\adv$'s winning probability is in fact overwhelming:
\begin{claim}
...
\begin{enumerate}
  ...
  \item $\Fipake$ generates key $SK$ which is equal to $SK^*$ if $\pw^* = \pw$ and uniformly random otherwise. $\Fipake$ then sends $SK$ to the UC protocol party $P$.
...
\begin{align*}
&\Pr[\env\text{ outputs }1\text{ in the ideal world}] \\
...
\[
\Pr[\widehat{\adv}\text{ wins}] \leq \frac{1}{2} + \frac{q}{2|D|} + \negl,
\]
